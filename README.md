...
| **Mnemoverse** | https://github.com/mnemoverse/mcp-memory-server | Hosted memory over MCP for coding agents: recall is re-ranked by feedback on whether a retrieved memory helped, unused memories decay by recency, and memory rooms are shared across accounts; ships the CC0 agent-memory-discipline skill for when to recall and when to save |
